...
Implement a server-side Adzuna adapter. Use environment credentials, configurable searches, pagination, timeout/retry, rate-limit awareness, structured logging and response validation. Map external jobs into internal contract. Do not scrape. Do not expose credentials. Record source metadata. Build mocked tests and a usage counter. Document that current terms/limits must be verified before commercial redistribution.
