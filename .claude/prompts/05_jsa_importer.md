...
Implement an idempotent JSA IVI importer for available occupation × state/territory and regional datasets. Parse official files safely, validate schemas, normalize occupation/geography references, preserve raw/source metadata, version imports and produce import summaries. Handle duplicate rows, missing values, changed column names and failed imports. Never label IVI as total vacancies. Add fixture-based tests.
