...
Create a stable geographic analytics API. Accept occupation, metric, period, comparison period and geography level. Validate all inputs. Return geography ID/name/parent, value, change, change percent, period, source, coverage and availability state. Distinguish zero from missing/unavailable. Keep aggregation server-side. Add caching where safe and tests for every state.
