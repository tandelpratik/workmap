...
Build salary parsing and analytics for reported individual-job salary data. Normalize currencies/periods, handle ranges, annual/hourly values and missing data. Calculate median/average only with sensible sample thresholds. Display sample size and source coverage. Do not invent salary or convert uncertain values without marking assumptions. Add parsing and statistical tests.
