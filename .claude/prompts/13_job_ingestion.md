# Prompt 13 — Generic Job Ingestion
...
Build a source-neutral JobSourceAdapter interface and ingestion pipeline: fetch → validate → normalize → deduplicate → upsert → track. Support batching, idempotency, retries, failure records and run metrics. Make it possible to add licensed sources without modifying the core pipeline. Add integration tests with mocked source adapters.
